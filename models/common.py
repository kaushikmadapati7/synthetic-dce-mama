"""Shared 2D building blocks used across the GAN and LDM models.

A 2D translation of the 3D pipeline's models/common.py: every Conv3d becomes Conv2d and
the depth dimension is dropped throughout. TimeEmbedding is dimension-agnostic and
copied verbatim.
"""
import math
...
class AttentionBlock2D(nn.Module):
    """Self-attention over flattened spatial plane."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # (b, heads, N, d) for fused attention; SDPA scales by 1/sqrt(d) internally and
        # never materializes the N x N matrix (the old softmax(q^T@k) OOM'd at 512^2,
        # where the latent grid is ~128^2 -> a 32 GiB attention matrix).
