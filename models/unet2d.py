...
2D translation of the 3D pipeline's models/unet3d.py; the forward/skip/attention logic
is dimension-agnostic and ported 1:1.
...
class UNet2D(nn.Module):
    def __init__(
        ...
        for i, out_ch in enumerate(chans):
            for _ in range(num_res_blocks):
                stage = nn.ModuleList([ResBlock2D(ch, out_ch, emb_dim)])
                ch = out_ch
                if i in attn_resolutions:
                    stage.append(AttentionBlock2D(ch))
                self.down.append(stage)
                skip_chs.append(ch)
            if i < len(chans) - 1:
                self.down.append(nn.ModuleList([Downsample2D(ch)]))
                skip_chs.append(ch)

        # ---- middle ----
        self.mid = nn.ModuleList([ResBlock2D(ch, ch, emb_dim),
                                  AttentionBlock2D(ch),
                                  ResBlock2D(ch, ch, emb_dim)])

        # ---- decoder ----
        self.up = nn.ModuleList()
        for i, out_ch in reversed(list(enumerate(chans))):
            for _ in range(num_res_blocks + 1):
                stage = nn.ModuleList([ResBlock2D(ch + skip_chs.pop(), out_ch, emb_dim)])
                ch = out_ch
                if i in attn_resolutions:
                    stage.append(AttentionBlock2D(ch))
                self.up.append(stage)
            if i > 0:
                self.up.append(nn.ModuleList([Upsample2D(ch)]))
    ...
    def forward(self, x, t, cond=None, labels=None):
        ...
        for stage in self.down:
            for layer in stage:
                h = layer(h, emb) if isinstance(layer, ResBlock2D) else layer(h)
            skips.append(h)
        ...
        for stage in self.up:
            first = stage[0]
            if isinstance(first, ResBlock2D):
