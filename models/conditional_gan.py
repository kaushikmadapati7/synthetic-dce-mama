...
Generator maps (noise z, condition c) -> image.
Discriminator scores (image, condition c) as real/fake (projection discriminator).

2D translation of the 3D pipeline's models/conditional_gan.py. For MAMA-SYNTH the condition
is the single pre-contrast slice (cond_channels=1) and the output is the single
post-contrast slice. The generator's final activation is configurable (default identity,
since data is z-scored rather than tanh-bounded [-1, 1]).
"""
import torch
...
class Generator2D(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.z_dim = z_dim
        # init_size may be an int (square) or an (H, W) tuple (anisotropic targets)
        self.init_size = (init_size,) * 2 if isinstance(init_size, int) else tuple(init_size)
        ...
        init_numel = self.init_size[0] * self.init_size[1]
        self.fc = nn.Linear(z_dim + cond_dim, chans[0] * init_numel)
        self.chans0 = chans[0]

        # if a conditioning image is provided, encode it and fuse at the bottleneck
        if cond_channels > 0:
            self.cond_enc = nn.Sequential(
                nn.Conv2d(cond_channels, chans[0], 3, padding=1),
                _norm(chans[0]), nn.SiLU(),
            )
            self.cond_pool = nn.AdaptiveAvgPool2d(self.init_size)
        ...
        self.to_out = nn.Sequential(nn.Conv2d(in_ch, out_channels, 3, padding=1),
                                    _final_activation(final_act))
...
class ConditionalGAN2D(nn.Module):
    """Convenience wrapper bundling G and D plus the sampling helper."""

    def __init__(self, **kwargs):
        super().__init__()
        g_keys = {"z_dim", "out_channels", "cond_channels", "num_classes",
                  "base_ch", "init_size", "n_upsamples", "final_act"}
        d_keys = {"in_channels", "cond_channels", "num_classes", "base_ch", "n_downsamples"}
        self.generator = Generator2D(**{k: v for k, v in kwargs.items() if k in g_keys})
        self.discriminator = Discriminator2D(**{k: v for k, v in kwargs.items() if k in d_keys})
