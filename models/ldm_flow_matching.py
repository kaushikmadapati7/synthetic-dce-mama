"""2D Latent Diffusion Model with a (rectified) flow-matching backbone.
...
2D translation of the 3D pipeline's models/ldm_flow_matching.py: the flow logic is
dimension-agnostic; only the UNet backbone is 2D.
"""
import torch
...
class LDM_FlowMatching(nn.Module):
    def __init__(
        ...
        self.unet = UNet2D(**(unet_kwargs or {}))
    ...
    # ---- training ----
    def loss(self, z0, cond=None, labels=None, x1=None):
        """z0: clean latent (t=0 -> data). t=1 endpoint is ``x1`` if given (a paired
        source latent, e.g. the pre-contrast, for I2SB-style bridge flow), else
        Gaussian noise (standard noise->data flow)."""
        b = z0.shape[0]
        t = torch.rand(b, device=z0.device)
        noise = x1 if x1 is not None else torch.randn_like(z0)
    ...
    def sample(self, shape, device, steps=50, cond=None, labels=None,
               solver="heun", decode=True, x1=None):
        # start the ODE at the t=1 endpoint: the paired source latent x1 (bridge) or noise
        z = x1 if x1 is not None else torch.randn(shape, device=device)
