"""Training loop for the 2D conditional GAN (pix2pix-style, single-stage)."""
...
from ..models import ConditionalGAN2D, d_hinge_loss, g_total_loss
...
def train_gan(args, train_loader, test_loader, criterion, device):
    ...
    gan = ConditionalGAN2D(z_dim=args.z_dim, out_channels=1, cond_channels=1,
                           base_ch=args.base_ch, init_size=init, n_upsamples=n_up,
                           in_channels=1, n_downsamples=n_down,
    ...
    if getattr(args, "eval_only", False):
        ckpt = args.ckpt or str(Path(args.output_dir) / "checkpoints" / "gan_last.pt")
        gan.load_state_dict(torch.load(ckpt, map_location=device))
        log.info(f"[eval-only] loaded {ckpt}")
        gen = lambda cond, mask=None: gan.sample(cond.size(0), device, cond_vol=cond)
        return gan, gen
